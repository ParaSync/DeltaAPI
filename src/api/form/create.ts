...
const normaliseComponentInput = (component: unknown, index: number): NormalisedComponent | null => {
  ...
  const mergedProperties: Record<string, unknown> = { ...baseProperties, ...settingsProperties };
...
const buildComponentResponse = (row: unknown): StoredComponent => {
  if (!isPlainObject(row)) throw new Error('Invalid component row');

  const properties = isPlainObject(row?.properties) ? row.properties : {};
  const derivedOrder = typeof properties.order === 'number' ? properties.order : toComponentOrder(row?.order, 0);
    ...
    try {
      ...
      for (const component of normalisedComponents) {
        const componentResult = await client.query(
          ...
          `,
          [formRow.id, { ...component.properties, order: component.order }, component.name || null, component.type]
        );
